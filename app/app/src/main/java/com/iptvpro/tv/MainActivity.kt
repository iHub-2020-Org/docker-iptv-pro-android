...
class MainActivity : Activity(), SurfaceHolder.Callback {
    ...
    private lateinit var tvNoChannelsHint: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        initViews()
        if (!Config.isConfigured(this)) openSettings()
        else loadChannels()
    }
    ...
    private fun initViews() {
        ...
        tvNoChannelsHint = findViewById(R.id.tvNoChannelsHint)
        ...
        listChannels.setOnItemClickListener { _, _, pos, _ ->
            currentChannelIndex = pos
            PlayListCache.saveLastChannelIndex(this, pos)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQ_SETTINGS) {
            channels.clear(); player?.stop(); player = null
            loadChannels()
            Toast.makeText(this, "服务器: ${Config.BASE_URL}", Toast.LENGTH_SHORT).show()
        }
    }

    // ─── Channel Loading: 三级降级策略 ────────────────────────────────────────
    private fun loadChannels() {
        // Level 1: 本地 SharedPreferences 缓存（立即显示，0延迟）
        ...
        if (cached.isNotEmpty()) {
            ...
            refreshChannelUI()
            Log.d(TAG, "Loaded ${channels.size} channels from local cache")
        }
        // Level 2 & 3: 后台从服务器刷新
    ...
    private fun fetchFromServer() {
        // Level 2: /api/results（内存，扫描后有效）
        try {
            val resp = ApiClient.getSync(Config.Endpoints.RESULTS) ?: throw Exception("no response")
            val arr = JSONObject(resp).optJSONArray("results")
            if (arr != null && arr.length() > 0) {
                val fresh = (0 until arr.length()).map { Channel.fromJson(arr.getJSONObject(it)) }
                mainHandler.post { applyChannels(fresh, "server-memory") }
                return
            }
        } catch (e: Exception) {
            Log.w(TAG, "Results fetch: ${e.message}")
        }

        // Level 3: /api/playlist/load（持久化，最可靠）
        try {
            // 先拿列表
            val listResp = ApiClient.getSync(Config.Endpoints.PLAYLIST_LIST)
                ?: throw Exception("playlist list unavailable")
            val playlists = JSONObject(listResp).optJSONArray("playlists")
            if (playlists == null || playlists.length() == 0) {
                mainHandler.post { showNoChannels("服务器无频道 — 按MENU扫描") }
                return
            }

            // 优先用上次用过的，否则用最新的
            val lastUsed = Config.getLastPlaylist(this)
            val filename = if (lastUsed != null && (0 until playlists.length()).any { playlists.getString(it) == lastUsed })
                lastUsed
            else
                playlists.getString(0)

            Log.d(TAG, "Loading playlist: $filename")
            val loadResp = ApiClient.getSync("${Config.Endpoints.PLAYLIST_LOAD}?filename=${filename}")
                ?: throw Exception("load failed")
            val results = JSONObject(loadResp).optJSONArray("results")
            if (results != null && results.length() > 0) {
                val fresh = (0 until results.length()).map { Channel.fromJson(results.getJSONObject(it)) }
                Config.saveLastPlaylist(this, filename)
                mainHandler.post { applyChannels(fresh, "playlist:$filename") }
            } else {
                mainHandler.post { showNoChannels("播放列表为空 — 按MENU扫描") }
            }
        } catch (e: Exception) {
    ...
    private fun applyChannels(fresh: List<Channel>, source: String) {
        if (fresh.size <= channels.size && channels.isNotEmpty()) return // cache is fresher
        Log.d(TAG, "Applied ${fresh.size} channels from $source")
    ...
    private fun showNoChannels(hint: String) {
        tvNoChannelsHint.text = hint
    ...
    private fun switchMode(mode: Int) {
        ...
        when (mode) {
            MODE_LIST -> { listChannels.requestFocus()
                if (currentChannelIndex < channelAdapter.count) listChannels.setSelection(currentChannelIndex) }
    ...
    private fun handlePlayKeys(k: Int): Boolean = when (k) {
        ...
        KeyEvent.KEYCODE_ENTER, KeyEvent.KEYCODE_DPAD_CENTER -> { if (channels.isNotEmpty()) switchMode(MODE_LIST); true }
        KeyEvent.KEYCODE_MENU -> { switchMode(MODE_SCAN); true }
        KeyEvent.KEYCODE_INFO -> { if (infoBar.visibility == View.VISIBLE) infoBar.visibility = View.GONE
            else if (channels.isNotEmpty()) showInfoBar(channels[currentChannelIndex]); true }
        KeyEvent.KEYCODE_0 -> { openSettings(); true }
        else -> super.onKeyDown(k, null)
    }

    private fun handleListKeys(k: Int): Boolean = when (k) {
    ...
    private fun handleScanKeys(k: Int): Boolean = when (k) {
        KeyEvent.KEYCODE_BACK, KeyEvent.KEYCODE_DPAD_LEFT -> { switchMode(MODE_PLAY); true }
        KeyEvent.KEYCODE_ENTER, KeyEvent.KEYCODE_DPAD_CENTER -> { if (isScanning) stopScan() else startScan(); true }
        KeyEvent.KEYCODE_0 -> { openSettings(); true }
        else -> super.onKeyDown(k, null)
    }
    ...
    private fun startScan() {
        ...
        tvScanStatus.text = getString(R.string.scanning); tvScanFound.text = ""
        ...
        ApiClient.postAsync(Config.Endpoints.SCAN_EXECUTE, config.toJson().toString()) { resp ->
            ...
            else { isScanning = false; tvScanStatus.text = "连接失败 — 按0检查设置" }
        }
    }
            ...
            when (event) {
                ...
                "scan_complete" -> mainHandler.post {
                    ...
                    if (channels.isNotEmpty()) mainHandler.postDelayed({
                        switchMode(MODE_PLAY); currentChannelIndex = 0; playCurrentChannel() }, 1000) }
    ...
    // ─── Lifecycle ───────────────────────────────────────────────────────────
    override fun onPause() { super.onPause(); player?.pause(); mainHandler.removeCallbacks(infoBarHideRunnable) }
    override fun onResume() { super.onResume(); player?.resume() }
    override fun onDestroy() {
        super.onDestroy(); sseClient?.disconnect(); player?.stop()
        mainHandler.removeCallbacksAndMessages(null); surfaceView.holder.removeCallback(this)
    }
}
